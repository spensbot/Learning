...
def getModel(attention_params: models.MultiHeadAttentionParams, layers: int):
    print(attention_params)
    n_chars = len(data.chars2)

    transformer = models.Transformer(
        n_tokens=n_chars, n_layers=layers, attention_params=attention_params
    )
    transformer.to(DEVICE)
    return transformer

def getBatch(dataSet, attention_params):
...
def train(model, trainData, testData, batch_count: int) -> list[float]:
    ...
    for batch in range(batch_count):
        X, Y = getBatch(trainData, model.attention_params)
...
def eval(model, data) -> None:
    with torch.no_grad():
        X, Y = getBatch(data, model.attention_params)
...
def generate(model, seed: str, count: int) -> str:
    n_l = model.attention_params.n_l
...
if __name__ == "__main__":
    model = getModel(models.MultiHeadAttentionParams(n_heads=4, n_qkv=64, masked=True), layers=2)
